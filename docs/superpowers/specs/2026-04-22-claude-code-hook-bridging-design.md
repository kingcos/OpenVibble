...
**零改动**。现有协议：
- `HeartbeatSnapshot.prompt: PendingPromptInfo`（`id` / `tool` / `hint`）
- Mac → iOS 广播 pending 时填 `hint = "[<projectName>] <原 hint>"`
- iOS Approve/Deny → BLE 发 `PermissionCommand` → Mac 已能收 → 转给 HookBridge 解析 race
