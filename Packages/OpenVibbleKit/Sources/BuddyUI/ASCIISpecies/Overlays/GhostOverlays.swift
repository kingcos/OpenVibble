...
enum GhostOverlays {
    ...
    // z DIM  at (67+18+p1, 6+16 - p1*2) where p1 = t%12
    // Z WHITE at (67+24+p2, 6+12 - p2)   where p2 = (t+4)%12
    // z DIM  at (67+14+p3/2, 6+8 - p3/2) where p3 = (t+8)%12
    ...
    // BUSY: BOO ticker — "o  ","oO ","oOo"," Oo","  o","   " at fixed cursor, cycled by t%6
    // CYAN at (67+22, 6+14)
    // tick%6==5 → blank "   ", no overlay needed
    private static let busy: [Overlay] = [
        Overlay(char: "o  ", tint: .rgb565(0x07FF), path: .fixed(col: gridCol(px: 67 + 22), row: gridRow(px: 6 + 14)), visibility: .tickMod(period: 6, activeTicks: [0])),
        Overlay(char: "oO ", tint: .rgb565(0x07FF), path: .fixed(col: gridCol(px: 67 + 22), row: gridRow(px: 6 + 14)), visibility: .tickMod(period: 6, activeTicks: [1])),
        Overlay(char: "oOo", tint: .rgb565(0x07FF), path: .fixed(col: gridCol(px: 67 + 22), row: gridRow(px: 6 + 14)), visibility: .tickMod(period: 6, activeTicks: [2])),
        Overlay(char: " Oo", tint: .rgb565(0x07FF), path: .fixed(col: gridCol(px: 67 + 22), row: gridRow(px: 6 + 14)), visibility: .tickMod(period: 6, activeTicks: [3])),
        Overlay(char: "  o", tint: .rgb565(0x07FF), path: .fixed(col: gridCol(px: 67 + 22), row: gridRow(px: 6 + 14)), visibility: .tickMod(period: 6, activeTicks: [4])),
    ]
