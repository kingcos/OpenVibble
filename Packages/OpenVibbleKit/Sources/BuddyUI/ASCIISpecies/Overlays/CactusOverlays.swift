...
enum CactusOverlays {
    ...
    // Z WHITE at (67+26+p2,   6+14 - p2)   where p2 = (t+4)%10
    ...
    // GREEN at (67+22, 6+14); DOTS[t % 6]
    // tick%6==5 → blank "   ", no overlay needed
    private static let busy: [Overlay] = [
        Overlay(char: ".  ", tint: .rgb565(0x07E0), path: .fixed(col: gridCol(px: 67 + 22), row: gridRow(px: 6 + 14)), visibility: .tickMod(period: 6, activeTicks: [0])),
        Overlay(char: ".. ", tint: .rgb565(0x07E0), path: .fixed(col: gridCol(px: 67 + 22), row: gridRow(px: 6 + 14)), visibility: .tickMod(period: 6, activeTicks: [1])),
        Overlay(char: "...", tint: .rgb565(0x07E0), path: .fixed(col: gridCol(px: 67 + 22), row: gridRow(px: 6 + 14)), visibility: .tickMod(period: 6, activeTicks: [2])),
        Overlay(char: " ..", tint: .rgb565(0x07E0), path: .fixed(col: gridCol(px: 67 + 22), row: gridRow(px: 6 + 14)), visibility: .tickMod(period: 6, activeTicks: [3])),
        Overlay(char: "  .", tint: .rgb565(0x07E0), path: .fixed(col: gridCol(px: 67 + 22), row: gridRow(px: 6 + 14)), visibility: .tickMod(period: 6, activeTicks: [4])),
    ]
    ...
    // colors cycle through YEL, HEART, CYAN, WHITE, PURPLE
    // char alternates * vs o by stream index
    ...
    // CYAN "*" at offset 0, YEL "*" at offset 4
