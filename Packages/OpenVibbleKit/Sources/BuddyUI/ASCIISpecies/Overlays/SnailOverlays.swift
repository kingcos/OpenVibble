...
enum SnailOverlays {
    ...
    // SLEEP: three Z-streams drift up-LEFT over 12 ticks (snails are slow, gentle Z's)
    // z DIM   at (67+8-p1, 6+14 - p1)   where p1 = t%12
    // Z WHITE at (67+14-p2, 6+10 - p2)  where p2 = (t+5)%12
    // z DIM   at (67+4-p3/2, 6+6 - p3/2) where p3 = (t+9)%12
    private static let sleep: [Overlay] = [
        Overlay(
            char: "z",
            tint: .dim,
            path: .linear(
                originCol: gridCol(px: 67 + 8),
    ...
    // IDLE: slow slime drip gated on (t/6)&1
    // CYAN "." at (67-10, BUDDY_Y_BASE+32) → period 12, active at {6..11}
    // Note: BUDDY_Y_BASE=30, so pixel y = 30+32=62; gridRow(62) = (62-30)/8 = 4.0
    private static let idle: [Overlay] = [
        Overlay(
            char: ".",
            tint: .rgb565(0x07FF), // BUDDY_CYAN
            path: .fixed(col: gridCol(px: 67 - 10), row: gridRow(px: 62)),
            visibility: .tickMod(period: 12, activeTicks: [6, 7, 8, 9, 10, 11])
        ),
    ]

    // BUSY: DOTS ticker — 6 chars at fixed cursor, cycled by t%6
    // CYAN at (67+24, 6+12); print DOTS[t % 6]
    // tick%6==5 → blank "   ", no overlay needed
    private static let busy: [Overlay] = [
        Overlay(char: ".  ", tint: .rgb565(0x07FF), path: .fixed(col: gridCol(px: 67 + 24), row: gridRow(px: 6 + 12)), visibility: .tickMod(period: 6, activeTicks: [0])),
        Overlay(char: ".. ", tint: .rgb565(0x07FF), path: .fixed(col: gridCol(px: 67 + 24), row: gridRow(px: 6 + 12)), visibility: .tickMod(period: 6, activeTicks: [1])),
        Overlay(char: "...", tint: .rgb565(0x07FF), path: .fixed(col: gridCol(px: 67 + 24), row: gridRow(px: 6 + 12)), visibility: .tickMod(period: 6, activeTicks: [2])),
        Overlay(char: " ..", tint: .rgb565(0x07FF), path: .fixed(col: gridCol(px: 67 + 24), row: gridRow(px: 6 + 12)), visibility: .tickMod(period: 6, activeTicks: [3])),
        Overlay(char: "  .", tint: .rgb565(0x07FF), path: .fixed(col: gridCol(px: 67 + 24), row: gridRow(px: 6 + 12)), visibility: .tickMod(period: 6, activeTicks: [4])),
    ]
    ...
    // CYAN "*" at (67 + OX[t%8] - 2, 6+4 + OY[t%8])  ← note: OY+4 not OY+6
    // YEL  "*" at (67 + OX[(t+4)%8] - 2, 6+4 + OY[(t+4)%8])
