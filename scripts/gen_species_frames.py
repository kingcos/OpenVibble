...
def emit_swift(species: list, output: Path) -> None:
    # Each species is emitted as its own private static let so the Swift
    # optimizer (esp. COWArrayOpt at -O whole-module) handles each init as a
    # small SIL function instead of one giant tree that stalls ColdBlockInfo.
    lines = [
    ...
    for sp in species:
        lines.append(f'        "{sp["name"]}": {_ident(sp["name"])},')
    lines.append("    ]")
    lines.append("")
    for sp in species:
        lines.append(
            f'    private static let {_ident(sp["name"])}: [PersonaState: SpeciesStateData] = ['
        )
...
def _ident(name: str) -> str:
    # Species keys are already lowercase identifiers; keep as-is.
    return name
