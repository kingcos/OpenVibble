...
object SnailOverlays {
    ...
    // IDLE: slow cyan slime drip gated on (t/6)&1.
    private val idle: List<Overlay> = listOf(
        Overlay(
            char = ".",
            tint = OverlayTint.Rgb565(0x07FF),
            path = OverlayPath.Fixed(col = gridCol(67 - 10), row = gridRow(62)),
            visibility = OverlayVisibility.TickMod(period = 12, activeTicks = listOf(6, 7, 8, 9, 10, 11)),
        ),
    )

    // BUSY: CYAN DOTS ticker.
    private val busy: List<Overlay> = run {
        val col = gridCol(67 + 24)
        val row = gridRow(6 + 12)
    ...
    // CELEBRATE: 6 confetti streams, char alternates * vs . by stream index.
    ...
    // DIZZY: two orbiting "*" stars. OY+4 anchor (not OY+6 like cat/capybara).
